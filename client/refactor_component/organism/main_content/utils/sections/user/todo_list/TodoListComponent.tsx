...
const TodoListComponent: React.FC<TodoListComponentProps> = ({ issueId }) => {
  ...
  const fetchTodos = async () => {
    try {
      const response = await fetch(
        `http://localhost:3001/user/${issueId}/todos`
      );
  ...
  const handleCompleteTodo = async (todoId: string) => {
    try {
      const response = await fetch(
        `http://localhost:3001/user/todos/${todoId}/complete`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleDeleteTodo = async (todoId: string) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:3001/user/todos/${todoId}`,
        {
          method: "DELETE",
        }
      );
      ...
      <ul>
        {todos.map((todo, index) => (
          <li key={todo.todo_id || `temp-${index}`}>
            ...
            {!todo.isComplete && (
              <button onClick={() => handleCompleteTodo(todo.todo_id)}>
                Complete
              </button>
            )}
