...
import ClockInButton from "../backButtonSection/ClockInButton"; // 출퇴근 버튼 컴포넌트를 임포트합니다.
  ...
  return (
    <>
      ...
      <div className={`${usersection} ${kanbansection}`}>
        <div className={cardHeader}>kanban board</div> {/* 칸반 보드의 제목 */}
      ...
      {/* 공지사항 게시판을 보여주는 섹션입니다. */}
      <div className={`${usersection} ${calendarsection}`}>
        <div className={projectTitle}>
          <span>Notice Board</span> {/* 공지사항 게시판 제목 */}
          <div className={tdn}>
            {/* 게시판 페이지로 이동하는 버튼 */}
            <Button onClick={() => onclick(<NoticeMainPage />)}>게시판</Button>
          </div>
        </div>
        <NoticeBoard /> {/* 공지사항 게시판 컴포넌트를 렌더링합니다. */}
      </div>

      {/* 할일 목록(To-Do List)을 보여주는 섹션입니다. */}
      <div className={`${usersection} ${todolistsection}`}>
        <div className={cardHeader}>todolist</div> {/* 할일 목록의 제목 */}
        <div className={cardContent}></div> {/* 할일 목록의 콘텐츠 */}
      </div>

      {/* 사용자 알림 섹션으로, 캘린더 컴포넌트를 포함합니다. */}
      <div className={`${usersection} ${usernoticesection}`}>
        <div className={cardHeader}>
          <CalendarComponent /> {/* 캘린더 컴포넌트를 렌더링합니다. */}
        </div>
      </div>
